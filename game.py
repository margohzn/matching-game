...
dogy = pygame.transform.scale(dog, (150,150))
...
caty = pygame.transform.scale(cat, (150,150))
...
#these are the images that wont move during the game (on right side of gamw window)
fixed_positions = {
    jamy: (450, 125),
    peppery: (450, 320),
    dogy: (400,500)
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if dragging and selected_image:
                mouse_pos = pygame.mouse.get_pos()
                if selected_image not in fixed_positions:  #Check if image is movable images on the right sude o=wont be able to move
